...
interface BookmarkListProps {
  ...
  onGoToBookmark: (time: number) => void;
}
        ...
        <ul className="space-y-3 overflow-y-auto flex-grow">
          {bookmarks.sort((a,b) => a.time - b.time).map((bookmark) => (
            <li key={bookmark.id} className="bg-gray-700/50 p-3 rounded-lg flex items-center gap-4">
              <button onClick={() => onGoToBookmark(bookmark.time)} className="font-mono text-lg text-teal-400 hover:text-teal-300 bg-gray-900/50 px-3 py-1 rounded-md">
              ...
              <input
                ...
                className="flex-grow bg-transparent focus:outline-none focus:ring-1 focus:ring-teal-500 rounded px-2 py-1 text-white"
              />
